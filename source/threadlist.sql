select
  ...
  (select group_concat('<li><a href="/!userinfo/'||nick||'">'||nick||'</a></li>','') from (select nick from threadposters left join users on userID = id where threadid = T.id order by firstPost)) as Posters,
