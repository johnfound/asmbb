<div class="thread_summary">
  <div class="col_unread">
    <a class="unread unread[case:[Unread]|0|1]" title="Mark thread as read." href="[case:[special:userid]|#|[Slug]/!markread]"></a>
    [case:[special:isadmin]||<a class="pinned pin[Pinned]" href="[Slug]/!pinit"></a>]
  </div>
  <div class="thread_link [case:[special:isadmin]|[case:[Pinned]||pin1|]|]">
    <a class="thread_link" href="[Slug]/">[Caption]</a><br>
    <span class="small">[special:posters=[id]]</span>
    <span class="small">tags: [special:threadtags=[id]]</span>
  </div>

  <div class="col_center">
    [PostCount] [case:[PostCount]|posts|post|posts]
    <div class="unread_cnt">([case:[Unread]| |<a href="/[FirstUnread]/!by_id" title="Go to first unread post.">][Unread] unread[case:[Unread]| |</a>])</div>
    <span class="small">[TimeChanged]</span>
  </div>
  <div class="col_center">
    [ReadCount]<br>[case:[ReadCount]|views|view|views]
  </div>
</div>
